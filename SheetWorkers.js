...
on("change:Development change:Population sheet:opened", function() {  //sheet:opened is optional, but is useful for some workers - the script runs every time the sheet opens.
    getAttrs(["Development","Population"], function(values) {
         ...
         let Population = parseInt(values.Population)||0;    // like so
            var calcValue = Development * Population * 500
         setAttrs({ 
 ...
 on("change:Law change:Population", function() {  //sheet:opened is optional, but is useful for some workers - the script runs every time the sheet opens.
    getAttrs(["Law","Population"], function(values) {
         let Law = parseInt(values.Law)||0;   // wrap this in a parseInt function if you need it to be a number
         let Population = parseInt(values.Population)||0;    // like so
            var calcValue = Law * Population * 500
         setAttrs({ 
                 LawUpkeep: calcValue                          
         });
   });
...
on("clicked:EndSeason", function(){
    getAttrs(["Population", "FoodStock", "MetalStock", "StoneStock","WoodStock","ExoticStock","treasury", "SeasonFood", "SeasonMetal", "SeasonStone","SeasonWood","SeasonExotic","SeasonRevenue"], function (values) {
        let food = (parseInt(values.FoodStock) || 0) + (parseInt(values.SeasonFood) || 0) - (parseInt(values.Population) || 0);
        let metal = (parseInt(values.MetalStock) || 0) + (parseInt(values.SeasonMetal) || 0);
        let stone = (parseInt(values.StoneStock) || 0) + (parseInt(values.SeasonStone) || 0);
        let wood = (parseInt(values.WoodStock) || 0) + (parseInt(values.SeasonWood) || 0);
        let exotic = (parseInt(values.ExoticStock) || 0) + (parseInt(values.SeasonExotic) || 0);
        let money = (parseInt(values.treasury) || 0) + (parseInt(values.SeasonRevenue) || 0);
        setAttrs({
         FoodStock : food,
         MetalStock : metal,
         StoneStock : stone,
